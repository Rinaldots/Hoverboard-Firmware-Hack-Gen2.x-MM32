...
template <typename O> static bool ReceiveTokenFrame(O& oSerial, SerialTokenPass& outFrame)
{
  while (oSerial.available() >= (int)sizeof(SerialTokenPass))
  {
    int first = oSerial.peek();
    if (first != '/')
    {
      return false;
    }
    uint8_t* raw = (uint8_t*)&outFrame;
    for (size_t i = 0; i < sizeof(SerialTokenPass); ++i)
    {
      int byteRead = oSerial.read();
      if (byteRead < 0)
      {
        return false;
      }
      raw[i] = (uint8_t)byteRead;
    }
    uint16_t crc = CalcCRC((uint8_t*)&outFrame, sizeof(SerialTokenPass) - 2);
    if (crc == outFrame.checksum)
    {
      return true;
    }
    // CRC failed; continue scanning
  }
  return false;
}
